...
function touchScreen(event){
    ...
    if (isToggleButton) {
        ...
        inImgBtn('.grid-button.primary','hoban.png');
 ...
 */
function inImgBtn(buttonSelector, imageName) {
    ...
    if (targetButton) {
        ...
        imgElement.style.width = '24px'; // 이미지 크기 조절
...
function createCharacterPopup() {
    // 팝업 컨테이너 생성
    characterPopup = document.createElement('div');
    ...
    messageText.id = 'message-text'; // 텍스트를 쉽게 바꿀 수 있도록 ID 부여
    messageText.textContent = "안녕하세요!";
    
    messageBubble.appendChild(messageText);

    // 캐릭터 이미지 생성
    const characterImg = document.createElement('img');
    // 'character.png'는 manifest.json의 web_accessible_resources에 등록해야 합니다.
    characterImg.src = chrome.runtime.getURL('images/character.png'); 

    // 컨테이너에 말풍선과 캐릭터 이미지 추가
    characterPopup.appendChild(messageBubble);
    characterPopup.appendChild(characterImg);
...
/**
 * 캐릭터 팝업을 보여주면서 메시지를 설정하는 함수
 * @param {string} message - 보여주고 싶은 메시지 텍스트
 * @param {number} [duration=3000] - 메시지를 보여줄 시간 (ms). 0이면 자동 숨김 없음.
 */
function showCharacterMessage(message, duration = 3000) {
    if (!characterPopup) return;

    // 1. 메시지 텍스트 변경
    const messageText = characterPopup.querySelector('#message-text');
    if (messageText) {
    ...
    // 3. 일정 시간 후 자동으로 숨기기 (duration이 0보다 클 때만)
    if (duration > 0) {
        setTimeout(() => {
